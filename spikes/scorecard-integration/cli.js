...
async function searchCommand(args) {
  ...
  try {
    ...
    console.log(
      `   Security:      ${formatScore(eval_.breakdown.security)} ${color('dim', '(OpenSSF Scorecard)')}`
    );
    console.log(
      `   Maintenance:   ${formatScore(eval_.breakdown.maintenance)} ${color('dim', '(Libraries.io)')}`
    );
    console.log(
      `   Popularity:    ${formatScore(eval_.breakdown.popularity)} ${color('dim', '(GitHub)')}`
    );
    console.log(
      `   Ecosystem:     ${formatScore(eval_.breakdown.ecosystem)} ${color('dim', '(Dependents)')}`
    );
...
async function auditCommand(args) {
  ...
  try {
    ...
    Object.entries(grades)
      ...
      .forEach(([grade, count]) => {
        const colorName =
          grade === 'A' ? 'green' : grade === 'B' || grade === 'C' ? 'yellow' : 'red';
  ...
  try {
    ...
    data.results.forEach((result, index) => {
      ...
      if (result.error) {
        ...
        console.log(
          `${medal} ${rank.toString().padEnd(3)} ${name} ${score} ${grade}      ${status}`
        );
      }
    });
...
${evaluated
  .sort((a, b) => b.score - a.score)
  .slice(0, 5)
  .map((r, i) => `${i + 1}. **${r.name}** - ${r.score.toFixed(1)}/10`)
  .join('\n')}
