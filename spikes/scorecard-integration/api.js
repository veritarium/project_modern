...
async function getScorecardData(owner, repo) {
  ...
  try {
    const output = execSync(`scorecard --repo=github.com/${owner}/${repo} --format=json`, {
      encoding: 'utf-8',
      timeout: 60000,
    });
...
async function getLibrariesIoData(platform, name) {
  ...
  try {
    ...
    const response = await fetch(`https://libraries.io/api/${platform}/${name}${apiKey}`, {
      timeout: 10000,
    });
...
function getMockLibrariesIoData(platform, name) {
  ...
  return {
    ...
    latest_release_published_at: new Date(
...
function calculateMaintenanceScore(librariesData) {
  ...
  if (librariesData.latest_release_published_at) {
    const daysSince =
      (Date.now() - new Date(librariesData.latest_release_published_at)) / (1000 * 60 * 60 * 24);
...
async function start() {
  ...
  fastify.log.info(
    `  Libraries.io API Key: ${CONFIG.librariesIoApiKey ? 'set' : 'not set (rate limited)'}`
  );
