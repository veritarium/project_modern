...
if (test('search without package shows error', () => {
  const result = runCommand('search');
  assert(!result.success, 'Command should fail');
  assert(result.output.includes('Error') || result.output.includes('required'), 'Should show error');
})) passed++; else failed++;

// Test 3: API connection check (will fail if API not running, that's OK)
if (test('search with package attempts API call', () => {
...
if (test('audit without package.json shows error', () => {
  const result = runCommand('audit --path /nonexistent/package.json');
  assert(!result.success, 'Command should fail');
  assert(result.output.includes('Cannot find') || result.output.includes('Error'), 'Should show file not found');
})) passed++; else failed++;

// Test 5: Compare with insufficient args shows error
if (test('compare with one package shows error', () => {
...
if (test('unknown command shows error', () => {
