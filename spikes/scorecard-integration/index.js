...
async function getScorecardData(owner, repo) {
  ...
  try {
    const output = execSync(
      `scorecard --repo=github.com/${fullName} --format=json`,
...
function calculateMaintenanceScore(data) {
  ...
  if (data.latest_release_published_at) {
    const daysSinceRelease = (Date.now() - new Date(data.latest_release_published_at)) / (1000 * 60 * 60 * 24);
...
function printResults(toolName, result) {
  ...
  if (result.details.libraries) {
    ...
    console.log(`   Dependents: ${result.details.libraries.dependent_packages_count?.toLocaleString() || 'N/A'}`);
