...
export class PackageHoverProvider implements vscode.HoverProvider {
  ...
  async provideHover(
    ...
    const _lineText = document.lineAt(position.line).text;
  ...
  private isInDependenciesSection(document: vscode.TextDocument, line: number): boolean {
    ...
    for (let i = line; i >= 0; i--) {
      ...
      if (lineText.match(/^\s*"[^"]+"\s*:\s*\{/) && !lineText.includes('dependencies')) {
        return false;
      }
    }
    return false;
  ...
  private createHoverContent(data: any): vscode.Hover {
    ...
    const _scoreColor = this.api.getScoreColor(eval_.composite);
    ...
    markdown.appendMarkdown(
      `- Maintenance: ${this.api.formatScore(eval_.breakdown.maintenance)}\n`
    );
    ...
    if (data.sources.libraries?.dependents) {
      markdown.appendMarkdown(
        `- 📦 Dependents: ${data.sources.libraries.dependents.toLocaleString()}\n`
      );
    }
